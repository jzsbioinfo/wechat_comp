...
train = load_pickle('data/wechat_algo_data1/jin_train.pkl')
#train.drop('user_date_weight_emd',axis = 1,inplace = True)
test = load_pickle('data/wechat_algo_data1/jin_test.pkl')
#test.drop('user_date_weight_emd',axis = 1,inplace = True)
cols = load_pickle('data/wechat_algo_data1/jin_cols.pkl')
...
for seed in [2021,10086,13579,9394,456][0:1]:
    ...
    print(uauc_list)
    print(weighted_uauc)
    logger.info(str(uauc_list))
    logger.info(str(weighted_uauc))
    ...
    for y in y_list[:4]:
        ...
        #保存模型
        if not os.path.exists('../../data/model/jin'):os.makedirs('../../data/model/jin')
        save_pickle(clf, '../../data/model/jin/seed_{}_day{}_train_model_{}.m'.format(seed,n_day,y))
        ##特征重要性
        feat = pd.DataFrame({
            'column': cols,
