...
for i in range(10):
    ...
    if ONLINE:
        for y in y_list[:4]:
            print('=========', y, '=========')
            t = time.time()
            clf = loadPkl('./src2/models/model_%s_seed%s.pkl' % (y, i))
            test[y] += clf.predict_proba(test[cols])[:, 1]/10
            print('runtime: {}\n'.format(time.time() - t))
        # test[['userid', 'feedid'] +
        #      y_list[:4]].to_csv('results/model_seed%s.csv' %
        #                         (i),
        #                         index=False)
...
for seed in [2021,10086,13579,9394,456]:
    ...
    #     '../data/submission/jin/seed_%d_day%d_sub_jin.csv' % (seed,n_day),
    #     index=False)
lgb3 = test
...
submit.to_csv('result.csv.csv', index=None)
