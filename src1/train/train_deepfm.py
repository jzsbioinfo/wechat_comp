...
ACTION_LIST = ["read_comment", "like", "click_avatar", "forward"]
submit = test_use_all[['userid', 'feedid']+ACTION_LIST].reset_index(drop=True)
...
for seed_num in seed_list:
    ...
    # print(score)
    # print(0.4*score[0]+0.3*score[1]+0.2*score[2]+0.1*score[3])

    # # 保存提交文件
    # import time
    # time_str = time.strftime('%d_%H_%M_%S', time.localtime(time.time())) 
    # submit.to_csv(f"./deepfm_testb_14_seed_{seed_num}_{time_str}.csv", index=False)
